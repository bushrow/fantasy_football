...
class League:
    ...
    def _get_scoring_period_data(self, scoring_period):
        logging.info(
            "Requesting boxscore data from ESPN for %d, period %d."
    ...
    def get_matchup(self, number: int, team_id: int, boxscore: bool = False) -> Matchup:
    ...
    def get_matchups_by_number(
        self, number: int, boxscore: bool = False
    ) -> List[Matchup]:
