...
class PlayerWeek:
    ...
    def __init__(self, stat_data: Dict[str, Any]) -> None:
        ...
        self.projected_points = proj.get("appliedTotal") or 0.0
        self._coded_stats, self._coded_proj = dict(), dict()
        self._coded_stats = {int(k): v for k, v in real["stats"].items()}
        if proj.get("stats"):
            self._coded_proj = {int(k): v for k, v in proj["stats"].items()}

    def __repr__(self):
        pts_str = "Inactive" if self.points is None else "%0.1f points" % self.points
        return "{} : {} : {}".format(self.slot, self.player, pts_str)
